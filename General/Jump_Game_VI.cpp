...
int solve(int idx, int maxSum, vector<int>&dp, vector<int>&nums, int k) {
	...
	for (int i = 1; i <= k; i++) {
		if (idx - k >= 0) {
			temp = max(temp, solve(idx - i, maxSum, dp, nums, k));
		}
